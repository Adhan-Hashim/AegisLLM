...
const timelineData = [
  ...
  { time: '10:00', allowed: 5100, blocked: 890 }, // Jailbreak attempt spike
  { time: '11:00', allowed: 4900, blocked: 340 },
  { time: '12:00', allowed: 5300, blocked: 1100 }, // Prompt Leakage spike
...
const topThreatsData = [
  { name: 'Prompt Injection', count: 1240, color: '#ef4444' }, // red-500
  { name: 'Jailbreak', count: 890, color: '#f97316' },        // orange-500
  { name: 'Prompt Leakage', count: 650, color: '#eab308' },   // yellow-500
  { name: 'PII/Secrets', count: 420, color: '#3b82f6' },      // blue-500
  { name: 'Toxicity', count: 210, color: '#8b5cf6' },         // purple-500
];
...
const recentIncidents = [
  { id: 1, time: '12:44', model: 'GPT-4.1', rule: 'PI-003', risk: 97, action: 'Blocked' },
  { id: 2, time: '12:41', model: 'Claude 3.5', rule: 'SEC-001', risk: 88, action: 'Blocked' },
  { id: 3, time: '12:38', model: 'Gemini 1.5', rule: 'JB-002', risk: 91, action: 'Blocked' },
  { id: 4, time: '12:30', model: 'Llama 3', rule: 'TOX-004', risk: 75, action: 'Flagged' },
  { id: 5, time: '12:15', model: 'GPT-4.1', rule: 'PI-001', risk: 100, action: 'Blocked' },
];
            ...
            <motion.div variants={itemVars} className="col-span-8 glass-card p-0 flex flex-col overflow-hidden">
              <div className="p-4 border-b border-border/50 bg-secondary/20">
                <h3 className="text-sm font-bold uppercase tracking-wider text-muted-foreground flex items-center gap-2">
                  <Clock className="w-4 h-4" /> Recent Incidents
                </h3>
              </div>
                ...
                <table className="w-full text-sm text-left">
                  ...
                  <tbody>
                    {recentIncidents.map((incident) => (
                      <tr key={incident.id} className="border-b border-border/20 hover:bg-secondary/10 transition-colors">
                        <td className="px-4 py-3 font-mono text-xs text-gray-400">{incident.time}</td>
                        <td className="px-4 py-3 font-semibold text-gray-200">{incident.model}</td>
                        <td className="px-4 py-3"><span className="px-2 py-1 rounded bg-secondary text-secondary-foreground text-xs font-mono">{incident.rule}</span></td>
                        <td className="px-4 py-3 text-center">
                          <span className={`font-bold ${incident.risk >= 90 ? 'text-destructive' : 'text-warning'}`}>{incident.risk}</span>
                        </td>
                        <td className="px-4 py-3 text-right">
                          <span className={`px-2 py-1 rounded text-xs font-bold uppercase tracking-wider ${incident.action === 'Blocked' ? 'bg-destructive/10 text-destructive border border-destructive/20' : 'bg-warning/10 text-warning border border-warning/20'}`}>
                            {incident.action}
                          </span>
                        </td>
                      </tr>
